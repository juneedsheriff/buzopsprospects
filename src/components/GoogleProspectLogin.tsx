...
export function GoogleProspectLogin({
  onSuccess,
  disabled = false,
  autoPrompt = false,
}: GoogleProspectLoginProps) {
  ...
  useGoogleOneTapLogin({
    ...
    onError: handleError,
    disabled: !autoPrompt || disabled || loading || !GOOGLE_CLIENT_ID,
  ...
  return (
    <div ref={wrapperRef} className="google-prospect-login relative h-10 w-full">
      <Button
        ...
        leftSection={<IconBrandGoogle size={18} />}
        variant="default"
        className="pointer-events-none h-10 rounded-[10px] border border-[#E2E8F0] bg-white text-[13px] font-semibold text-[#0F172A]"
      >
        Continue with Google
      </Button>
